...
import { Github, Linkedin, Plus, LogIn } from 'lucide-react'
...
export default function Home() {
  ...
  const onGenerate = async () => {
    try {
      ...
      alert('Failed to generate Room ID. Ensure server is running.')
    }
  }
  ...
  const onCreate = async () => {
    if (!displayName.trim()) return alert('Enter your display name')
    ...
    try {
      ...
      alert('Failed to create room. Ensure server is running.')
    }
  }
  ...
  const onJoin = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${base}/api/rooms/${joinRoomId.trim()}/validate`)
      if (res.data?.exists) navigate(`/room/${joinRoomId.trim()}`)
      else alert('Room not found. Ask interviewer for a valid Room ID.')
    } catch {
      alert('Validation failed. Ensure server is running.')
    }
  }
      ...
      <div className="mx-auto max-w-6xl px-6 pt-20 pb-16">
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }} className="glass-panel rounded-lg p-6 md:p-7">
            <h2 className="text-lg font-semibold mb-4 text-gray-100">Join a Room</h2>
            <div className="space-y-2">
              <Input placeholder="Room ID" value={joinRoomId} onChange={(e) => setJoinRoomId(e.target.value)} invalid={!!errors.room} />
              {errors.room && <div className="text-red-400 text-xs px-1">{errors.room}</div>}
              <Input placeholder="Display name" value={displayName} onChange={(e) => setDisplayName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') onJoin() }} invalid={!!errors.name} />
              {errors.name && <div className="text-red-400 text-xs px-1">{errors.name}</div>}
            </div>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Button size="lg" onClick={onJoin} className="inline-flex items-center justify-center"><LogIn className="mr-2 h-4 w-4" />Join</Button>
              <Button size="lg" variant="secondary" onClick={onGenerate} className="inline-flex items-center justify-center"><Plus className="mr-2 h-4 w-4" />Generate ID</Button>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.05 }} className="glass-panel rounded-lg p-6 md:p-7 relative overflow-hidden">
            ...
            <div className="relative">
              <h2 className="text-lg font-semibold mb-3 text-gray-100">Host an Interview</h2>
